...
function Userlistdisplay({ selected, report, ref }) {
  const selectedmodule = selected;
  let reportdatasource = report;
  if (reportdatasource === undefined) {
    reportdatasource = [];
  }

  //To get the search value
  const { Search } = Input;
  //state variable to filterted user detail
  const [filteredUserDetails, setFilteredUserDetails] = useState([]);
  //State variable to store the search value
  const [searchText, setSearchText] = useState("");
  //State Variables to Store the user data fetched from getuserdetail API
  const [userdetails, setUserdetails] = useState([]);
  const [DataSource, setDatasource] = React.useState([]);
  // Updating filteredUserDetails whenever userdetails or searchText changes
  useEffect(() => {
  ...
  //State Variables to store the Updated Datas
  const [Firstname, setFirstname] = React.useState("");

  const [Lastname, setLastname] = React.useState("");
  const [Phonenumber, setPhonenumber] = React.useState("");
  const [Aadharnumber, setAaadharnumber] = React.useState("");
  const [Email, setEmail] = React.useState("");
  const [Address, setAddress] = React.useState("");
  const [Courseamount, setCourseamount] = React.useState("");
  const [Remainingamount, setRemainingamount] = React.useState("0");
  const [Updatedremainingamount, setUpdatedremainingamount] = React.useState(0);
  ...
  const [updatedPaymentstatus, setupdatedPaymentstatus] = React.useState("");
  ...
  const [Firstinstallment, setFirstinstallment] = React.useState("");
  const [UpdatedSecondinstallment, setUpdatedSecondinstallment] =
    React.useState(0);
  const [UpdatedThirdinstallment, setUpdatedThirdinstallment] =
    React.useState(0);
  ...
  const [updatedtrainerid, setupdatedTrainerid] = React.useState("");
  const [iscompletedpayment, setiscompletedpayment] = React.useState(false);
  const [trainername, setTrainername] = useState("");
  // eslint-disable-next-line no-unused-vars
  const [isActive, setisActive] = useState(true);
  const [filteredreport, setFilteredreport] = useState([]);

  const fetchData = async (trainerid, Userchoosencourse) => {
    try {
      ...
      const trainerdetails = users.find((user) => user._id === trainerid);

      if (trainerdetails) {
        // Set trainer details if found
        setTrainername(trainerdetails.Firstname);
      } else {
        // If the above request fails, try fetching using Userchoosencourse
        try {
          const response = await fetch(
            `${process.env.REACT_APP_BACKEND_URL}/gettrainerdetail/${Userchoosencourse}`
          );
          const user = await response.json();
          setTrainername(user[0].Firstname);
          setupdatedTrainerid(user[0]._id);
        } catch (error) {
          //console.log("Error fetching trainer details by course:", error);
        }
      }
    } catch (error) {
  ...
  //Function to get the user entered value for Updating the User data
  const getvaluetoupdate = async (id) => {
    setOpen(true); // Popup the form

    const user = userdetails.find((user) => user._id === id);
    setuserdetailToUpdate(user.Email);
    ...
    if (user) {
      ...
      setAaadharnumber(user.Aadharnumber);
      setEmail(user.Email);
      setAddress(user.Address);
      setCourseamount(user.Courseamount);
      setDuration(user.Duration);
      setRole(user.Role);
      setUserchoosencourse(user.Userchoosencourse);
      setRemainingamount(user.Remainingamount);
      setFirstinstallment(user.Firstinstallment);
      setUserstatus(user.Userstatus);
      fetchData(user.Trainerid, user.Userchoosencourse);

      if (user.Paymentstatus === "Paid") {
        setiscompletedpayment(true);
      }
      if (user.Secondinstallment > 0) {
        setSecondinstallment(user.Secondinstallment);
        setUpdatedSecondinstallment(user.Secondinstallment);
        setThirdinstallment(user.Thirdinstallment);
        setIsDisabled(false);
      } else {
        setSecondinstallment(user.Remainingamount);
        setIsDisabled(true);
        setThirdinstallment(0);
      }
      if (user.Thirdinstallment > 0) {
      } else {
        setThirdinstallment(user.Remainingamount);
      }
      let dateValue = user.Dateofjoining;
      if (dateValue) {
        const parsedDate = dayjs(dateValue);
        if (parsedDate.isValid()) {
          setDateofjoining(parsedDate);
        } else {
          //console.log("Invalid Date format in Dateofjoining.");
        }
      } else {
        //console.log("Dateofjoining is missing or invalid.");
      }
    } else {
  ...
  const handlesecondinstallment = (e) => {
    const secondInstallment = parseFloat(e.target.value) || 0;
    setSecondinstallment(secondInstallment);
    setUpdatedSecondinstallment(secondInstallment);
    setUpdatedThirdinstallment(0);
    const PaidAmount = Firstinstallment + secondInstallment;
    const balance = Courseamount - PaidAmount;
    if (PaidAmount === Courseamount) {
      setupdatedPaymentstatus("Paid");
      setUpdatedremainingamount(0);
      setUpdatedThirdinstallment(0);
      setThirdinstallment(0);
    } else if (PaidAmount >= Courseamount) {
      toast.warning("Invalid Amount Value");
      handleClickOpen();
    } else {
      setupdatedPaymentstatus("Partially Paid");
      setUpdatedremainingamount(balance);
    }

    if (isDisabled) {
      setUpdatedThirdinstallment(0);
    } else {
      setUpdatedThirdinstallment(Updatedremainingamount);
    }
  };

  const handlethirdinstallment = (e) => {
    const thirdInstallment = parseFloat(e.target.value) || 0;
    setThirdinstallment(thirdInstallment);
    const PaidAmount = Firstinstallment + Secondinstallment + thirdInstallment;
    if (PaidAmount === Courseamount) {
      setupdatedPaymentstatus("Paid");
    } else if (PaidAmount >= Courseamount) {
      toast.warning("Invalid Amount Value");
    ...
    if (isDisabled) {
      ...
      setUpdatedThirdinstallment(thirdInstallment);
    }
  };
  ...
  const handlestatus = (checked, id) => {
    if (checked) {
      ...
      status(id, "Inactive", false);
    }
  };

  const status = async (id, Userstatus, isActive) => {
    const userdetail = userdetails.find((Admin) => Admin._id === id);
    const Email = userdetail.Email;
    try {
      const Response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/updateuserstatus/${Email}`,
        {
          method: "PUT", // Using PUT method to update the value
          credentials: "include",
          headers: {
  ...
  const Update = async (event) => {
    ...
    //To check for empty values before updating the user data
    if (
      ...
      try {
        const Response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/updateuser/${userdetailtoUpdate}`,
          {
            ...
            body: JSON.stringify({
              Secondinstallment: UpdatedSecondinstallment,
              Thirdinstallment: UpdatedThirdinstallment,
              Remainingamount: Updatedremainingamount,
              Paymentstatus: updatedPaymentstatus,
              Trainerid: updatedtrainerid ? updatedtrainerid : undefined,
            }),
          }
        );
        if (Response.status === 200) {
          ...
          handleClose();
          setUpdatedSecondinstallment("");
          setUpdatedThirdinstallment(0);
          setUpdatedremainingamount("");
    ...
    } else if (updatedtrainerid !== "") {
      try {
        const Response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/updateusertrainer/${userdetailtoUpdate}`,
          {
            method: "PUT", //PUT Method to update the value
            headers: {
              "Content-Type": "application/json",
  ...
  const Delete = async (id) => {
    ...
    try {
      if (window.confirm("Are you sure you want to delete?")) {
        const Response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/deleteuser/${Email}`,
          {
  ...
  const handlePrint = (id) => {
    ...
    if (user) {
      const totalAmount =
        user.Firstinstallment + user.Secondinstallment + user.Thirdinstallment;
      const toWords = new ToWords();
      const numberinwords = toWords.convert(totalAmount, { currency: true });
            ...
            <div class="invoice-container">
              <div class="invoice-header">
                <div>
                  ...
                  <br />
                  <span>
                    <span class="companyname">NOTASCO TECHNOLOGIES INDIA PRIVATE LIMITED</span>
                    <br />
              ...
              <div class="invoice-body">
                ...
                <div>
                  <span>
                    <span style="font-weight: lighter; text-transform: uppercase;">AMOUNT DUE:</span>
                    <br />
                    <span style="color: red; font-weight: bolder; font-size: 14px;">
                      ₹${Remainingamount}
                    </span>
                    <br />
                    <span style="font-size: 10px;">Issued on: ${istTimestamp}</span>
                  </span>
                </div>
              </div>
              <br />
              <div>
                <table>
                  ...
                  <tbody>
                    <tr>
                      <td>1</td>
                      <td>Training fee (1st Installment)</td>
                      <td align="right">₹${user.Courseamount}</td>
                    ...
                    <tr>
                      <td>2</td>
                      <td>Training fee (2nd Installment)</td>
                      <td align="right">₹${user.Courseamount}</td>
                      <td align="right">${user.Duration}</td>
                      <td align="right" style="font-weight: bold;">₹${
                    ...
                    <tr>
                      <td>3</td>
                      <td>Training fee (3rd Installment)</td>
                      <td align="right">₹${user.Courseamount}</td>
                      <td align="right">${user.Duration}</td>
                      <td align="right" style="font-weight: bold;">₹${
                        user.Thirdinstallment
                      }</td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div class="totals-container">
                ...
                <span style="font-weight: bolder;">
                  Total Amount:
                  <span style="font-weight: bold;">₹${user.Courseamount}</span>
                </span>
              </div>
              <div class="terms">
                <span style="font-weight: normal; padding-bottom: 30px; text-transform: capitalize;">
                  Total in Words: ${numberinwords}
                </span>
              </div>
      
              <div class="terms">
                Terms and Condition: Fees once paid shall not be refunded under any circumstances
              </div>
            </div>
          </div>
          <footer style="font-size: 12px;text-align:right;margin-top:60px;font-weight:bold">
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "Duration",
      sorter: (a, b) =>
        a.Duration.split(" ")[0].localeCompare(b.Duration.split(" ")[0]),
    },

    {
      title: "Technology",
      dataIndex: "Userchoosencourse",
      key: "Userchoosencourse",
      sorter: (a, b) =>
        a.Userchoosencourse.split(" ")[0].localeCompare(
          b.Userchoosencourse.split(" ")[0]
        ),
    },
    {
      title: "Paid/Unpaid",
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Switch
            onChange={(e) => handlestatus(e.target.checked, record._id)}
  ...
  //Using useeffect Hook to fetch the User data from Mongo db through getuserdetail API
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/getuserdetail`
        );
        const users = await response.json();
        setisActive(users.isActive);
  ...
  useEffect(() => {
    const filteredData = reportdatasource.filter((user) =>
      Object.values(user).some((field) =>
        String(field).toLowerCase().includes(searchText.toLowerCase())
      )
    );
    ...
    if (filteredData.length > 0) {
      setFilteredreport(filteredData); // Update the filteredreport state
    } else if (filteredData.length === 0) {
      setFilteredreport([]); // Reset the filteredreport state
    }
  }, [reportdatasource, searchText]);

  //To Display Loading text until the data's are fetched from getcourse API
  if (loading) {
    return (
      <div>
        <Loading></Loading>
  ...
  const tabledisplay = selectedmodule === "Report";
  const reportcolumn = [
    {
      title: "Firstname",
      dataIndex: "Firstname",
  ...
  return (
    <div>
      <img
        ...
        style={{ display: "none" }}
      />
      <div style={{ display: tabledisplay ? "none" : "block" }}>
        <Search
        ...
        {filteredUserDetails.length === 0 ? (
          ...
          <Table
            dataSource={
              filteredUserDetails.length > 0 ? filteredUserDetails : DataSource
            }
            style={{
              maxHeight: "500px",
              overflowY: "auto",
      ...
      <div style={{ display: tabledisplay ? "block" : "none" }}>
        <Search
          ...
          style={{
            ...
            zIndex: "2",
            display:
              filteredreport.length === 0 && searchText === ""
                ? "none"
                : "block",
          }}
        ></Search>
        ...
        {filteredreport.length === 0 ? (
          ...
          <Table
            dataSource={
              filteredreport.length > 0 ? filteredreport : reportdatasource
            }
          ...
          <DialogContent>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DemoContainer components={["DatePicker"]}>
                <DatePicker label="Date of Joining" value={Dateofjoining} />
              </DemoContainer>
            ...
            <br></br>
            <InputLabel>Choosen Course</InputLabel>
            <TextField
              autoFocus
              required
              margin="dense"
              id="Userchoosencourse"
              name="Userchoosencourse"
              type="String"
              fullWidth
              variant="outlined"
            ...
            <TextField
              ...
              onChange={(e) => handlesecondinstallment(e)}
            />
            <br />
            <TextField
              autoFocus
              ...
              variant="standard"
              disabled={isDisabled}
              value={isDisabled ? 0 : Thirdinstallment}
              onChange={(e) => handlethirdinstallment(e)}
            />
          </DialogContent>
          <DialogActions>
